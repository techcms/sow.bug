<?php
// we need a clean fast die.
if (!isset($_GET['problem'])) die('Ming the Merciless has terminated this action - what is your problem?');
$type_of_problem = $_GET['problem'];
...
try{
    ...
    if($type_of_problem=='htaccess'){
            ...
            if($folders = opendir($content_folder)){
                ...
                <script>
                    jQuery('input#mp-import-html').live('click',function(e){
                        ...
                        jQuery.ajax({
                            url:mp_root_url+'mp-includes/ajax/import-html.php',
                            data:({ nonce: nonce }),
                            type: "POST",
                            dataType: 'json',
        ...
        <IfModule mod_rewrite.c>
            # Used by PHP later to know if it is on or not
        ...
        # END MongoPress Rules
                ...
                if($progress != false){
                    ...
                    if($got_content_folder){
                        $problem_text.= '<br><p>'.__('Additionally, you may wish to consider importing the available content template.').'</p>';
                        $problem_text.= '<br><br><p>'.__('Please login to do this.').'</p>';
			$problem_text .= mp_get_login_form();
			$problem_text .=  "<script src='".$options['root_url']."mp-includes/js/ready.js'></script>";
                    } if($object_count>0){
                    ...
                    $problem_text = '<p>'.__('You may attempt to modify the permissions of your home directory with the following command, then reload this page').'</p>';
                    $problem_text .= '<pre>chmod 777 '.$options['root_path'].'</pre><br><br>';
                    ...
                    if($got_content_folder){
                        $problem_text.= '<p>'.__('Additionally, you may wish to consider importing the available content template.').'</p><br>';
                        $problem_text.= $login_form;
			$problem_text .=  "<script src='".$options['root_url']."mp-includes/js/ready.js'></script>";
                    }
        ...
        $error_content = '<h4>'.__('IT APPEARS YOU ARE MISSING THE "config.php" FILE').'</h4><p>'.__('If this is your first time, you probably just need to rename config-sample.php to config.php, <strong>but before you do</strong>, please be sure to make the necessary changes and properly configure things.').'</p>';
        ...
        if(file_exists(dirname(dirname(__FILE__)).'/mp-settings/config.php')){
            ...
            $object_count = $objects->find()->count();
            $m->close();
            setcookie('mp_logged_in_'.$options['cookie_salt'],false,-1,'/');
            setcookie('mp_logged_in_user_id_'.$options['cookie_salt'],1,-1,'/');
            mp_get_simple_header(__('Installation'));
                ...
                echo '<article><header><h3 class="article-title header">'.__('Installing MongoPress').'</h3></header><div class="content">';
                    try{
                        ...
                        if(!$errors){
                            ...
                            if(is_writable(dirname(dirname(__FILE__)).'/')){
                                echo '<h3 class="article-title header">'.__('Please Note That The Root Folder Has Write Permissions Enabled - You Really Should Resolve This').'</h3>';
                            }
                            ...
                            $object_count = $objects->find()->count();
                                if($type_of_problem!='missing_objs'){
                                    if($user_count<1){
                                        echo '<p>'.__('No User Accounts Created Yet').'</p>';
                                    ...
                                    mp_nonce_field('add-user','mp_nonce');
                                }
                                if($object_count<1){
