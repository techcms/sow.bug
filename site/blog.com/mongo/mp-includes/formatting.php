 ...
 */
function mptexturize($text) {
	global $mp_cockneyreplace;
	...
	if (!$static_setup) {
		...
		if ( isset($mp_cockneyreplace) ) {
			$cockney = array_keys($mp_cockneyreplace);
			$cockneyreplace = array_values($mp_cockneyreplace);
		} else {
	...
	// Transform into regexp sub-expression used in _mptexturize_pushpop_element
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( !empty($curl) && '<' != $curl{0} && '[' != $curl{0}
			...
			/*
			 * Only call _mptexturize_pushpop_element if first char is correct
			 * tag opening
			 */
			if ('<' == $curl{0})
				_mptexturize_pushpop_element($curl, $no_texturize_tags_stack, $no_texturize_tags, '<', '>');
			elseif ('[' == $curl{0})
				_mptexturize_pushpop_element($curl, $no_texturize_shortcodes_stack, $no_texturize_shortcodes, '[', ']');
		}
 ...
 * @return object
 */
function _mptexturize_pushpop_element($text, &$stack, $disabled_elements, $opening = '<', $closing = '>') {
...
function wpautop($pee, $br = 1) {
	...
	$allblocks = '(?:table|thead|tfoot|caption|col|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|select|option|form|map|area|blockquote|address|math|style|input|p|h[1-6]|hr|fieldset|legend|div|article|aside|hgroup|header|footer|nav|figure|figcaption|details|menu|summary)';
 ...
 */
function _mp_specialchars( $string, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	...
	// Store the site charset as a static to avoid multiple calls to mp_load_alloptions()
        /*
	if ( !$charset ) {
		static $_charset;
		if ( !isset( $_charset ) ) {
			$alloptions = mp_load_alloptions();
	...
	// Handle double encoding ourselves
	if ( !$double_encode ) {
		$string = str_replace( array( '|mp_entity|', '|/mp_entity|' ), array( '&', ';' ), $string );
	}
 ...
 * @param mixed $quote_style Optional. Converts double quotes if set to ENT_COMPAT, both single and double if set to ENT_QUOTES or none if set to ENT_NOQUOTES. Also compatible with old _mp_specialchars() values; converting single quotes if set to 'single', double if set to 'double' or both if otherwise set. Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function mp_specialchars_decode( $string, $quote_style = ENT_NOQUOTES ) {
	$string = (string) $string;
	...
	// Match the previous behaviour of _mp_specialchars() when the $quote_style is not an accepted value
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
	} elseif ( !in_array( $quote_style, array( 0, 2, 3, 'single', 'double' ), true ) ) {
		$quote_style = ENT_QUOTES;
 ...
 */
function mp_check_invalid_utf8( $string, $strip = false ) {
	$string = (string) $string;
	...
	static $is_utf8;
	if ( !isset( $is_utf8 ) ) {
                /* TODO: UNREMOVE PERHAPS ...?
		$is_utf8 = in_array( get_option( 'blog_charset' ), array( 'utf8', 'utf-8', 'UTF8', 'UTF-8' ) );
                --> QUICK FIX TO PREVENT CONFLICT WITH MONGOPRESS */
	}
	if ( !$is_utf8 ) {
		return $string;
	}
...
function sanitize_user( $username, $strict = false ) {
	...
	$username = mp_strip_all_tags( $username );
 ...
 */
function sanitize_title_with_dashes($title,$remove_dots=true) {
	$title = strip_tags($title);
...
function convert_chars($content, $deprecated = '') {
	...
	// Fix Word pasting
	$content = strtr($content, $mp_htmltranswinuni);
 ...
 */
function mp_rel_nofollow( $text ) {
	// This is a pre save filter, so text is already escaped.
	$text = stripslashes($text);
	$text = preg_replace_callback('|<a (.+?)>|i', 'mp_rel_nofollow_callback', $text);
	$text = esc_sql($text);
	return $text;
}
 ...
 */
function mp_rel_nofollow_callback( $matches ) {
	$text = $matches[1];
...
function translate_smiley($smiley) {
	...
	return " <img src='$srcurl' class='wp-smiley' ".mp_get_attr_filter('formatting.php','img',$srcurl,'','wp-smiley',"alt='$smiley_masked")." /> ";
}
 ...
 *
 * @uses $mp_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies($text) {
	global $mp_smiliessearch;
	$output = '';
	if ( get_option('use_smilies') && !empty($mp_smiliessearch) ) {
		...
		for ($i = 0; $i < $stop; $i++) {
			...
			if ((strlen($content) > 0) && ('<' != $content{0})) { // If it's not a tag
				$content = preg_replace_callback($mp_smiliessearch, 'translate_smiley', $content);
			}
			$output .= $content;
		}
	} else {
 ...
 * @usedby mp_mail() handles charsets in email subjects
 *
 * @param string $string Subject line
 * @return string Converted string to ASCII
 */
function mp_iso_descrambler($string) {
 ...
 */
function mp_trim_excerpt($text) {
	$raw_excerpt = $text;
	if ( '' == $text ) {
		$text = '';
		//$text = strip_shortcodes( $text );
		$text = apply_filters('mp_shortcodes', $text);
		$text = str_replace(']]>', ']]&gt;', $text);
	...
	return apply_filters('mp_trim_excerpt', $text, $raw_excerpt);
}
 ...
 */
function mp_htmledit_pre($output) {
	if ( !empty($output) )
 ...
 * @uses mp_kses_bad_protocol() To only permit protocols in the URL set
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	//if ( mp_kses_bad_protocol( $url, $protocols ) != $url )
		//return '';
...
function esc_js( $text ) {
	$safe_text = mp_check_invalid_utf8( $text );
	$safe_text = _mp_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = mp_check_invalid_utf8( $text );
	$safe_text = _mp_specialchars( $safe_text, ENT_QUOTES );
 ...
 */
function mp_make_link_relative( $link ) {
...
function sanitize_option($option, $value) {
	...
	switch ( $option ) {
		...
		case 'blogname':
			...
			$value = mp_filter_post_kses( $value ); // calls stripslashes then addslashes
			$value = stripslashes($value);
		...
		case 'upload_path':
			...
			$value = addslashes($value);
			$value = mp_filter_kses($value); // calls stripslashes then addslashes
			$value = stripslashes($value);
			break;
 ...
 * @uses apply_filters() for the 'mp_parse_str' filter.
 *
...
function mp_parse_str( $string, &$array ) {
	...
	$array = apply_filters( 'mp_parse_str', $array );
}
 ...
 *
 * @uses mp_pre_kses_less_than_callback in the callback function.
 *
 * @param string $text Text to be converted.
 * @return string Converted text.
 */
function mp_pre_kses_less_than( $text ) {
	return preg_replace_callback('%<[^>]*?((?=<)|>|$)%', 'mp_pre_kses_less_than_callback', $text);
}
 ...
 */
function mp_pre_kses_less_than_callback( $matches ) {
...
function mp_sprintf( $pattern ) {
	...
	while ( $len > $start ) {
		...
		if ( $pattern{$start} == '%' ) {
			...
			$_fragment = apply_filters( 'mp_sprintf', $fragment, $arg );
 ...
 */
function mp_sprintf_l($pattern, $args) {
	// Not a match
	...
	$l = apply_filters('mp_sprintf_l', array(
 ...
 * @return string The excerpt.
 */
function mp_html_excerpt( $str, $count ) {
	$str = mp_strip_all_tags( $str, true );
 ...
 * @return string The processed string.
 */
function mp_strip_all_tags($string, $remove_breaks = false) {
...
function sanitize_text_field($str) {
	$filtered = mp_check_invalid_utf8( $str );

	if ( strpos($filtered, '<') !== false ) {
		$filtered = mp_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = mp_strip_all_tags( $filtered, true );
	} else {
...
function capital_P_dangit( $text ) {
    ...
    if ( 'the_title' === current_filter() )
        return str_replace( 'Mongopress', 'MongoPress', $text );
    // Still here? Use the more judicious replacement
    static $dblq = false;
    if ( false === $dblq )
        $dblq = _x('&#8220;', 'opening curly quote');
    return str_replace(
        array( ' Mongopress', '&#8216;Mongopress', $dblq . 'Mongoress', '>Mongopress', '(Mongopress' ),
        array( ' MongoPress', '&#8216;MongoPress', $dblq . 'MongoPress', '>MongoPress', '(MongoPress' ),
    $text );
}

function mp_strip_mail_injection($str) {
    $injections = array(
        '/(\n+)/i',
        '/(\r+)/i',
	'/(\t+)/i',
...
function mp_attr_filter($file_name=false,$tag_type=false,$url=false,$id=false,$classes=false,$attributes=''){
    $attributes = mp_get_attr_filter($file_name,$tag_type,$url,$id,$classes,$attributes);
    echo $attributes;
}
function mp_get_attr_filter($file_name=false,$tag_type=false,$url=false,$id=false,$classes=false,$attributes=''){
    $filtered_attributes = apply_filters('mp_attributes',$attributes,$file_name,$tag_type,$url,$id,$classes);
    return $filtered_attributes;
}

function mp_filter_remove_shortcodes($content){
	$filtered_content = trim(preg_replace('/\[.*\]/', '', $content));
