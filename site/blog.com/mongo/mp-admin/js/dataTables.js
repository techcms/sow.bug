...
var initDataTables = function(src){
    var nonce = jQuery('form#objects-form').find('input[name="mp_nonce"]').val();
    var allow_edits = jQuery('form#objects-form').attr('data-allow-edits');
    if(!src){src=mp_root_url+'mp-includes/pjax/table.php'}
    $('table#objects').dataTable({
        ...
        "fnServerData":     function(sSource, aoData, fnCallback) {
            aoData.push({"name": "collection", "value": "objs"},{"name": "nonce", "value": nonce}, {"name": "use_mongo_meantime", "value": "true"}, {"name": "allow_edits", "value": allow_edits});
        ...
        "oLanguage": {
            "sProcessing": mp_languages.sProcessing,
            "sLengthMenu": mp_languages.sLengthMenu,
            "sZeroRecords": mp_languages.sZeroRecords,
            "sEmptyTable": mp_languages.sEmptyTable,
            "sInfo": mp_languages.sInfo,
            "sInfoEmpty": mp_languages.sInfoEmpty,
            "sInfoFiltered": mp_languages.sInfoFiltered,
            "sSearch": mp_languages.sSearch,
            "oPaginate": {
                    "sFirst":    mp_languages.oPaginate_sFirst,
                    "sPrevious": mp_languages.oPaginate_sPrevious,
                    "sNext":     mp_languages.oPaginate_sNext,
                    "sLast":     mp_languages.oPaginate_sLast
            }
        },
        "aoColumns": [
            {"sTitle": "<input type='checkbox' name='delete-all-objects' id='delete-all-objects' value='delete-all' class='delete-all' data-form='object' />", "sClass": "w5 unselectable"},
            {"sTitle": mp_languages.object_type, "sClass": "w15"},
            {"sTitle": mp_languages.slug, "sClass": "w20 unselectable"},
            {"sTitle": mp_languages.title, "sClass": "w25"},
            {"sTitle": mp_languages.created, "sClass": "center w15"},
            {"sTitle": mp_languages.updated, "sClass": "center w15"},
            {"sTitle": mp_languages.actions, "sClass": "center w10 last-column unselectable"}],
        "aoColumnDefs": [
    ...
    jQuery('a.edit-object').live('click',function(e){
        e.preventDefault();
		var this_element = $(this);
        var form_id = jQuery(this).attr('data-form');
        var this_form = jQuery('form#'+form_id);
        var mongoID = jQuery(this).attr('data-mongo-id');
        var nonce = jQuery('form#objects-form').find('input[name="mp_nonce"]').val();
        jQuery(this_form).find('span.notification').removeClass('errors');
        jQuery(this_form).find('span.notification').removeClass('success');
        jQuery(this_form).find('span.notification').html('Edit Object:');
		jQuery(this_element).addClass('loading');
        jQuery.ajax({
            url:mp_root_url+'mp-includes/pjax/get-object.php',
            data:({mongo_id: mongoID, nonce: nonce, use_mongo_meantime: 'true'}),
            type: "POST",
            dataType: 'json',
            success: function(result){
                if(result.success!=true){
                    alert(mp_languages.error);
                }else{
                    var type = result.type;
                    var slug_id = result.slug_id;
                    var title = result.title;
                    var content = result.content;
                    var custom = result.custom;
                    var custom_fields = mp_js_object_count(custom);
                    jQuery('div#mp-object-custom-wrapper').find('div.mp-custom-field-wrapper').remove();
                    jQuery(this_form).find('.button.hidden').removeClass('hidden');
					jQuery(this_element).removeClass('loading');
					if(is_ios===true){
						myScroll.scrollToElement('#widget-wrapper-add-edit-objects',500);
					}else{
						jQuery('#mp-content').scrollTo('#widget-wrapper-add-edit-objects',500);
					}
                    if(custom_fields>0){
                        var i=0;
                        var new_nonce = jQuery('form#object').find('input[name="mp_nonce"]').val();
                        for(var key in custom){
                            jQuery.ajax({
                                url:mp_root_url+'mp-includes/pjax/new-custom-field-row.php',
                                data:({i: i, nonce: new_nonce, key: key, value: custom[key]}),
                                type: "POST",
                                dataType: 'json',
                                success: function(result){
                                    if(result.success){
                                        jQuery('div#mp-object-custom-wrapper').prepend(result.content);
                                        jQuery('div#mp-object-custom-wrapper').attr('data-i',i);
                                        jQuery('a#mp-object-add-custom-field').removeClass('hidden');
                                    }
                                }
                            });i++;
                        }jQuery('div#mp-object-custom-wrapper').show(0);
                    }else{
                        jQuery('div#mp-object-custom-wrapper').hide(0);
                        jQuery('a#mp-object-add-custom-field').addClass('hidden');
                    }
                    jQuery(this_form).find('input#mongo_id').val(mongoID);
                    jQuery(this_form).find('input#type').val(type);
                    jQuery(this_form).find('select#type').val(type);
                    jQuery(this_form).find('input#slug_id').val(slug_id);
                    if(jQuery(this_form).find('div#temporary-slug').html()){

                    jQuery(this_form).find('div#mp-object-slug-wrapper').hide("normal",function(){
                        jQuery(this_form).find('input#slug').val('');
                        jQuery(this_form).find('div#temporary-slug').hide(0);
                        jQuery(this_form).find('label#object-slug-label').hide(0);
                        jQuery(this_form).find('a#mp-object-slug-switcher').hide(0);

                    });
                    }
                    jQuery(this_form).find('input#title').val(title);
                    jQuery(this_form).find('textarea#content').val(content);
                }
            },
            failure: function(){
                alert(mp_languages.error);
    ...
    jQuery('a.delete-object').live('click',function(e){
        e.preventDefault();
        if(confirm(mp_languages.confirmation_delete)) {
            ...
            jQuery.ajax({
                url:mp_root_url+'mp-includes/pjax/delete-object.php',
                data:({mongo_id: mongoID, nonce: nonce}),
                type: "POST",
                dataType: 'json',
                success: function(result){
                    if(result.success!=true){
                        jQuery(this_form).find('span.notification').removeClass('success');
                        jQuery(this_form).find('span.notification').addClass('errors');
                        jQuery(this_form).find('span.notification').html(result.message);
                        jQuery(this_form).find('span.notification').animate({'opacity':0},0,function(){
                        ...
                        jQuery(this_form).find('span.notification').removeClass('errors');
                        jQuery(this_form).find('span.notification').addClass('success');
                        jQuery(this_form).find('span.notification').html(result.message);
                        jQuery('table#objects').dataTable().fnDeleteRow(thisRow, null, true);
                        jQuery(this_form).find('span.notification').animate({'opacity':0},0,function(){
                            jQuery(this).animate({'opacity':1},'normal',function(){
                                /* AFTER ANIMATION */
                            });
                ...
                failure: function(){
                    jQuery(this_form).find('span.notification').removeClass('success');
                    jQuery(this_form).find('span.notification').addClass('errors');
                    jQuery(this_form).find('span.notification').html(mp_languages.error_finding_file);
                }
            });
        }
    });
    jQuery('form#object input.button').live('click',function(e){
        e.preventDefault();
        jQuery(this).addClass('hidden');
        var button_type = jQuery(this).attr('data-button-type');
        var form_id = jQuery(this).attr('data-form');
        var this_form = jQuery('form#'+form_id);
        jQuery(this_form).find('span.notification').removeClass('errors');
        jQuery(this_form).find('span.notification').removeClass('success');
        jQuery(this_form).find('span.notification').html(mp_languages.add_new_object);
        if(button_type=='add-new-object'){
            jQuery(this_form).find('input#id').val('');
            jQuery(this_form).find('input#mongo_id').val('');
            jQuery(this_form).find('input#type').val('');
            jQuery(this_form).find('select#type').val('');
            jQuery(this_form).find('input#slug').val('');
            jQuery(this_form).find('input#slug_id').val('');
            jQuery(this_form).find('input#title').val('');
            jQuery(this_form).find('textarea#content').val('');
            jQuery('textarea#content').wysiwyg('setContent','');
        }
    });
    jQuery('a#mp-admin-logout').live('click',function(e){
        e.preventDefault();
        var this_button = jQuery(this);
        jQuery(this_button).addClass('loading_light');
        jQuery.ajax({
            url:mp_root_url+'mp-includes/ajax/logout.php',
            type: "POST",
            dataType: 'json',
            success: function(result){
                jQuery(this_button).removeClass('loading_light');
                window.location = result.referrer;
            }
        });
    });
    jQuery('select#object-actions').live('click',function(){
        var action = jQuery(this).val();
        if((action=='delete')&&(jQuery('input.delete-me:checked').length>0)){
            var delete_me_count = jQuery('input.delete-me:checked').length;
            if(confirm(mp_languages.confirmation_delete)) {
                jQuery('input.delete-me').each(function(index){
                    var checked = jQuery(this).attr('checked');
                    if(checked){
                        var mongo_id = jQuery(this).attr('data-mongo-id');
                        if(action=='delete'){
                            var form_id = jQuery(this).attr('data-form');
                            var this_form = jQuery('form#'+form_id);
                            var mongoID = jQuery(this).attr('data-mongo-id');
    ...
    jQuery('input#delete-all-objects').live('change',function(){
        if(jQuery(this).attr('checked')){
            jQuery('input.delete-me').each(function(){
