...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		// Wrap all options
		if ( ($value['type'] != "block_begin") && ($value['type'] != "block_end") && ($value['type'] != "heading") && ($value['type'] != "info") ) {
		...
		case "images":
			...
			foreach ( $value['options'] as $key => $option ) {
				...
				$output .= '<img src="' . esc_url( dt_unify_url($option) ) . '" alt="' . $option .'" class="of-radio-img-img' . $selected .'" onclick="document.getElementById(\''. esc_attr($value['id'] .'_'. $key) .'\').checked=true;" />';
			}
		break;
		...
		case "block_begin":
			...
			if( isset( $value['id'] ) ){
				$id .= ' id="' . preg_replace('/\W/', '', strtolower($value['id']) ) . '"'; 
			}
			$output .= '<div' .$id. ' class="' . esc_attr( $class ) . '">'."\n";
			if( isset($value['name']) && !empty($value['name']) ){
				$output .= '<h4 class="block-heading">' . esc_html( $value['name'] ) . '</h4>' . "\n";
			}
		break;
