...
abstract class Daemon
{
    ...
    public function pid()
    {
        if (file_exists($this->_options['pid']) && $pid = intval(file_get_contents($this->_options['pid']))) {
            return $pid;
        }
    ...
    {
        $this->log('Starting daemon...', self::LOG_ALL);

        $this->_daemonize();

        $this->log('Daemon #' . $this->pid() . ' is running', self::LOG_ALL);
    ...
    public function log($msg, $io = self::LOG_FILE)
    {
        ...
        if (self::LOG_ECHO == $io || self::LOG_ALL == $io) {
            echo $msg, "\n";
        }

        if (self::LOG_FILE == $io || self::LOG_ALL == $io) {
    ...
    protected function _check()
    {
        ...
        if (!defined('SIGHUP')) { // Check for pcntl
            $this->log('PHP is compiled without --enable-pcntl directive', self::LOG_ALL);
            return false;
        }

        if ('cli' !== php_sapi_name()) { // Check for CLI
            $this->log('You can only create daemon from the command line (CLI-mode)', self::LOG_ALL);
            return false;
        }

        if (!function_exists('posix_getpid')) { // Check for POSIX
            $this->log('PHP is compiled without --enable-posix directive', self::LOG_ALL);
            return false;
        }
    ...
    protected function _fork()
    {
        ...
        if (-1 == $pid) { // error
            $this->log('Could not fork', self::LOG_ALL);
            return false;
        }
    ...
    protected function _stop()
    {
        if (!is_writeable($this->_options['pid'])) {
            $this->log('Daemon (no pid file) not running', self::LOG_ALL);
            return false;
        }

        $pid = $this->pid();
        unlink($this->_options['pid']);
        $this->log('Daemon #' . $pid . ' has stopped', self::LOG_ALL);
        $this->_exit = true;
    }
    ...
    protected function _restart()
    {
        global $argv;
        $this->_stop();
        $this->log('Daemon is restarting...', self::LOG_ALL);
        $cmd = $_SERVER['_'] . ' ' . implode(' ', $argv);
        if (false === strpos($cmd, ' > /dev/null 2>&1 &')) {
            $cmd .= ' > /dev/null 2>&1 &';
        }
        shell_exec($cmd);
    }
