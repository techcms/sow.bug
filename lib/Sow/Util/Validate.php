...
class Validate
{
    ...
    public function check($data, $rules, $ignorNotExists = false)
    {
            ...
            $rule = $rules + array('required' => false, 'msg' => self::$_message);

           
            $result = $this->_check($data, $rule);

            if (0 !== $result['code']) $this->_error = $result['msg'];

            if (isset($rule['rules'])) {
                $this->check($data, $rule['rules'], $ignorNotExists);
            }
